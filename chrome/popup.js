$(function () {setTimeout(function () {
    ...
    var sanitizeFavicon = function(url, iconUrl) {
        ...
        return '_favicon/?pageUrl=' + encodeURIComponent(url) + '&size=16';
    };
    
    var getWindowName = function (name) {
        if (!localStorage.tempWindowNames) {
    ...
    chrome.windows.getAll({populate: true}, function (windows) {
        windows.forEach(function (curWindow) {
            ...
            curWindow.tabs.forEach(function (curTab) {
                ...
                var favicon = sanitizeFavicon(curTab.url, curTab.favIconUrl);
    ...
    $(document).on('click', '.tabimglocal', function (e) {
        if (e.button === 0 && e.ctrlKey === false) {
            var tabId = parseInt($(this).attr('id').substring(7), 10);
            
            var windowAttr = $(this).attr('windowid'); 
            
            chrome.tabs.update(tabId, {active: true});
            
            if (windowAttr && windowAttr.indexOf('winl') === 0) {
                var windowId = parseInt(windowAttr.substring(4), 10);
    ...
    $(document).on('click', '.windowopen', function (e) {
        var windowId = parseInt($(this).parent().parent().attr('id').substring(4),10);
        chrome.runtime.sendMessage({tabs: TCWindows[windowId].tabs});
    });
                ...
                if (data.status == 'loggedin') {
                    if (data.windows.length == 0) {
                        ...
                        data.windows.forEach(function (curWindow) {
                            ...
                            var ti = 0;
                            curWindow.tabs.forEach(function (curTab) {
                                var favicon = sanitizeFavicon(curTab.url, curTab.favicon);
